	...
	Example:
		...
		"{\
			...
			{\
				...
				result = LkSendJsonCommand(error, connectionInfo, command, 0);
...
{
	return LkSendCommand(error, connectionInfo, command, EnvelopeFormatTYPE_JSON, receiveTimeout);
}
	...
	Example:
		...
		"<COMMAND NAME=\"READ\">\
			...
			<FILE_NAME>LK.CUSTOMERS</FILE_NAME>\
				...
				result = LkSendXmlCommand(error, connectionInfo, command, 0);
				LkLogout(error, connectionInfo, "", 0);
				LkFreeMemory(connectionInfo);
			}
			return result;
...
{
	return LkSendCommand(error, connectionInfo, command, EnvelopeFormatTYPE_XML, receiveTimeout);
}