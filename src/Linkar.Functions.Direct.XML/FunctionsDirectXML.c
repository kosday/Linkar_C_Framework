...
/*
	...
	Arguments:
		...
		xmlFormat - Specifies the desired output format: standard XML, XML_DICT format, or XML_SCH format
...
/*
	...
	Example:
		...
		char* MyLkProperties(char** error)
		{
			...
			result = LkProperties(error, credentials, "LK.CUSTOMERS", options, XML_FORMAT_XML, "", 600);
			LkFreeMemory(options);
			LkFreeMemory(credentials);
			return result;
		}
	...
	See Also:
		...
		<Release Memory>
*/
DllEntry char* LkProperties(char** error, const char* const credentialOptions, const char* const filename, const char* const lkPropertiesOptions, XML_FORMAT xmlFormat, const char* const customVars, uint32_t receiveTimeout)
{
	char* result = Base_LkProperties(error, credentialOptions, filename, lkPropertiesOptions, xmlFormat, customVars, receiveTimeout);
