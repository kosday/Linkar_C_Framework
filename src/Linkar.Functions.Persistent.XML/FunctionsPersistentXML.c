	...
	Example:
		...
		char* MyUpdatePartial(char** error)
		{
			...
			if(error == NULL && connectionInfo != NULL)
			{
				char* options = LkCreateUpdateOptions(FALSE, TRUE, TRUE, FALSE, FALSE, FALSE);
				XML_FORMAT xmlFormat = XML_FORMAT_XML;
				result = LkUpdatePartial(error, connectionInfo, "LK.CUSTOMERS",
		"<?xml version=\"1.0\" encoding=\"utf-16\"?>\
		<LINKAR>\
			<RECORDS>\
				<RECORD>\
...
{
	DataFormatTYPE inputFormat = DataFormatTYPE_XML;		
	char* result = Base_LkUpdatePartial(error, connectionInfo, filename, records, "", updateOptions, inputFormat, xmlFormat, customVars, receiveTimeout);
	...
	Example:
		...
		char* MyLkProperties(char** error)
		{
			...
			if(error == NULL && connectionInfo != NULL)
			{
				...
				result = LkProperties(error, connectionInfo, "LK.CUSTOMERS", options, XML_FORMAT_XML, "", 0);
				LkFreeMemory(options);
				LkLogout(error, connectionInfo, "", 0);
				LkFreeMemory(connectionInfo);
			}
...
{
	char* result = Base_LkProperties(error, connectionInfo, filename, lkPropertiesOptions, xmlFormat, customVars, receiveTimeout);
